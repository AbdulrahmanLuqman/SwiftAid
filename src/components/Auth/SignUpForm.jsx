...
import { toast } from "react-toastify"
  ...
  const SignUp = () => {
    ...
    const validateEmailAndPassword = (email, password)=> {
      ...
      if (!emailRegex.test(email)) {
        return { isValid: false, message: "invalid email address. Must include an @" }
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-[20.07px] pt-[20.07px] pb-[40.15px]">
      ...
      <h2 className="font-bold text-[33.46px] text-[#212121] w-[250px] max-[250px]:w-full">Create your Account</h2>
      <div className="flex flex-col gap-[26.77px]">
        <div className="w-full h-[50.19px] bg-[#FAFAFA] px-[16.73px] rounded-[10px] flex items-center gap-[10px]">
          ...
          <input onChange={handleChange} name="email" value={formData.email} id="email" placeholder="Nidaniaz@domain.com" type="text" className="font-semibold text-[11.71px] outline-none w-full h-full bg-transparent placeholder:text-[#212121]" />
        </div>
        <div className="w-full h-[50.19px] bg-[#FAFAFA] px-[16.73px] rounded-[10px] flex items-center gap-[10px]">
          <label htmlFor="account"><AccountType /></label>
          <p className="font-semibold text-[11.71px] ">Select Account Typee</p>
          {/* <input name="email"  type="text" className="font-semibold text-[11.71px] outline-none w-full h-full bg-transparent" /> */}
        </div>
        <div className="w-full h-[50.19px] bg-[#FAFAFA] px-[16.73px] rounded-[10px] flex items-center gap-[10px]">
          <label htmlFor="number"><Number /></label>
          <input onChange={handleChange} onKeyDown={(e)=> {if(e.key === "-" || e.key === "e") e.preventDefault() }} name="number" value={formData.number} id="number" placeholder="+1 234 2345" type="number" className="font-semibold text-[11.71px] outline-none w-full h-full bg-transparent placeholder:text-[#212121]" />
        </div>
        <div className="w-full h-[50.19px] bg-[#FAFAFA] px-[16.73px] rounded-[10px] flex items-center justify-between">
          <div className="flex items-center gap-[10px] w-full h-full">
            ...
            <input onChange={handleChange} name="password" value={formData.password} id="password" placeholder="********" type={!showPassword ? "password" : "text"} className="font-semibold text-[11.71px] outline-none w-full h-full bg-transparent placeholder:text-[#212121]" />
          </div>
        ...
        <div className="flex items-center gap-[10px]">
          <input onChange={toggleCheckbox} type="checkbox" id="remember" checked={isChecked} className="hidden" />
          <div onClick={toggleCheckbox} className="border-[2.51px] border-[#A5C339] w-5 h-5 rounded-[6px] p-[2px] cursor-pointer">
            <div className={isChecked ? "bg-[#A5C339] w-full h-full rounded-[50%]" : "hidden"}></div>
          </div>
          <label htmlFor="remember" className="text-[11.71px] font-semibold text-[#212121] cursor-pointer">Remember me</label>
        </div>
        <button className="bg-[#A5C339] w-full py-[15.6px] px-[13.87px] text-[#FFFFFF] text-center text-[13.87px] font-bold rounded-[73.87px]">Sign Up</button>
      </div>
